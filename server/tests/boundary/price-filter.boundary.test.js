...
test('maxPrice boundary: excludes a flight exactly 1 rupee above the threshold, includes one exactly at it', async (t) => {
  await t.test('[API-BND-026] maxPrice one below the cheapest flight excludes everything', async () => {
    const res = await searchDelBlr({ maxPrice: 4998 });
    assert.equal(res.body.count, 0);
  });

  await t.test('[API-BND-027] maxPrice exactly equal to the cheapest flight includes just that one', async () => {
    const res = await searchDelBlr({ maxPrice: 4999 });
    assert.equal(res.body.count, 1);
    assert.equal(res.body.flights[0].price, 4999);
  });

  await t.test('[API-BND-028] maxPrice exactly equal to the most expensive flight includes all three', async () => {
...
test('minPrice boundary: mirrors maxPrice at the opposite edge', async (t) => {
  await t.test('[API-BND-029] minPrice one above the most expensive flight excludes everything', async () => {
    const res = await searchDelBlr({ minPrice: 5900 });
    assert.equal(res.body.count, 0);
  });

  await t.test('[API-BND-030] minPrice exactly equal to the most expensive flight includes just that one', async () => {
    const res = await searchDelBlr({ minPrice: 5899 });
    assert.equal(res.body.count, 1);
    assert.equal(res.body.flights[0].price, 5899);
  });

  await t.test('[API-BND-031] minPrice of 0 excludes nothing', async () => {
    const res = await searchDelBlr({ minPrice: 0 });
    assert.equal(res.body.count, 3);
  });
});

test('[API-BND-032] minPrice === maxPrice (a single exact price point) returns only an exact match', async () => {
  const res = await searchDelBlr({ minPrice: 5399, maxPrice: 5399 });
  assert.equal(res.body.count, 1);
  assert.equal(res.body.flights[0].price, 5399);
});

test('[API-BND-033] an inverted range (minPrice > maxPrice) returns an empty result set rather than an error', async () => {
