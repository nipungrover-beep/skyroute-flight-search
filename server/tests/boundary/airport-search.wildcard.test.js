...
test('empty query returns the default airport list, capped at 8', async (t) => {
  await t.test('[API-BND-057] no q parameter at all', async () => {
    const res = await search();
    assert.equal(res.status, 200);
    assert.equal(res.body.length, 8);
  });

  await t.test('[API-BND-058] an explicit empty string', async () => {
    const res = await search('');
    assert.equal(res.body.length, 8);
  });

  await t.test('[API-BND-059] whitespace-only input is trimmed to empty', async () => {
    const res = await search('   ');
    assert.equal(res.body.length, 8);
  });
});

test('[API-BND-060] single-character wildcard matches both an airport code prefix and a city substring', async () => {
...
test('[API-BND-061] airport code matching is prefix-only, not substring', async () => {
...
test('[API-BND-062] city and name matching is substring-anywhere, not prefix-only', async () => {
...
test('[API-BND-063] matching is case-insensitive', async () => {
...
test('[API-BND-064] an exact full match returns exactly that airport', async () => {
  const res = await search('BLR');
  assert.equal(res.body.length, 1);
  assert.equal(res.body[0].code, 'BLR');
});

test('[API-BND-065] a query with no matches anywhere returns an empty array, not an error', async () => {
  const res = await search('xyzzy');
  assert.equal(res.status, 200);
  assert.deepEqual(res.body, []);
});

test('[API-BND-066] a query longer than any seeded field returns an empty array without crashing', async () => {
  const res = await search('a'.repeat(500));
  assert.equal(res.status, 200);
  assert.deepEqual(res.body, []);
});

test('[API-BND-067] special characters in the query are treated as literal text, not a pattern, and cannot crash the endpoint', async () => {
...
test('[API-BND-068] results are capped at 8 even when a broad query matches more', async () => {
