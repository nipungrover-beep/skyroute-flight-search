...
test('date boundary: yesterday is rejected, today and tomorrow are accepted', async (t) => {
  await t.test('[API-BND-017] yesterday (one day before the boundary) is rejected', async () => {
    const res = await search(isoDate(-1));
    assert.equal(res.status, 400);
    assert.match(res.body.error, /cannot be in the past/i);
  });

  await t.test('[API-BND-018] today (exact boundary) is accepted', async () => {
    const res = await search(isoDate(0));
    assert.equal(res.status, 200);
  });

  await t.test('[API-BND-019] tomorrow (one day after the boundary) is accepted', async () => {
    const res = await search(isoDate(1));
    assert.equal(res.status, 200);
  });
});

test('[API-BND-020] date format boundary: missing leading zeros are rejected', async () => {
...
test('calendar-validity boundary: a date matching the format regex but not a real calendar date is rejected', async (t) => {
  await t.test('[API-BND-021] month 13 does not exist', async () => {
    const res = await search(`${FUTURE_BASE_YEAR}-13-01`);
    assert.equal(res.status, 400);
    assert.match(res.body.error, /not a valid calendar date/i);
  });

  await t.test('[API-BND-022] day 45 does not exist in any month', async () => {
    const res = await search(`${FUTURE_BASE_YEAR}-01-45`);
    assert.equal(res.status, 400);
    assert.match(res.body.error, /not a valid calendar date/i);
  });

  await t.test('[API-BND-023] February 30th does not exist', async () => {
    const res = await search(`${FUTURE_BASE_YEAR}-02-30`);
    assert.equal(res.status, 400);
    assert.match(res.body.error, /not a valid calendar date/i);
  });

  await t.test('[API-BND-024] February 29th on a leap year is accepted', async () => {
    const res = await search(`${LEAP_YEAR}-02-29`);
    assert.equal(res.status, 200);
  });

  await t.test('[API-BND-025] February 29th on a non-leap year is rejected', async () => {
